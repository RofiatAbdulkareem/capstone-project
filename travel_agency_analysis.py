...
query_views = "SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' AND table_type = 'VIEW';"
...
currency_data = pd.read_sql("SELECT * FROM public.currency_count_by_continent;", engine)

# Save Bar Plot for currency_count_by_continent
plt.figure(figsize=(10, 6))
sns.barplot(x='continent', y='total_currencies', data=currency_data, hue='continent', palette="Set2")
...
population_density_data = pd.read_sql("SELECT * FROM public.population_density;", engine)

# Sorting data to get the top 10 highest and lowest population density
top_population_density = population_density_data.sort_values(by='population_density', ascending=False).head(10)
bottom_population_density = population_density_data.sort_values(by='population_density', ascending=True).head(10)

# Plot for Top 10 Countries by Population Density
plt.figure(figsize=(12, 6))
sns.barplot(x='country_name', y='population_density', data=top_population_density, palette="Greens_d")
...
plt.figure(figsize=(12, 6))
sns.barplot(x='country_name', y='population_density', data=bottom_population_density, palette="Reds_d")
...
start_of_week_data = pd.read_sql("SELECT * FROM public.start_of_week_analysis;", engine)

# Save Bar Plot for start_of_week_analysis
plt.figure(figsize=(8, 5))
sns.barplot(x='start_of_week', y='country_count', data=start_of_week_data, hue='start_of_week', palette="Blues_d")
...
# Population plot
sns.barplot(x='region', y='total_population', data=regions_data, ax=axes[0], hue='region', palette="Blues_d")
axes[0].set_title("Total Population by Region")
axes[0].set_xlabel("Region")
axes[0].set_ylabel("Total Population")

# Area plot
sns.barplot(x='region', y='total_area', data=regions_data, ax=axes[1], hue='region', palette="Greens_d")
...
languages_data = pd.read_sql("SELECT * FROM public.top_languages_by_region;", engine)

# Save Bar Plot for top_languages_by_region
plt.figure(figsize=(12, 6))
sns.barplot(x='region', y='language_count', data=languages_data, hue='languages', palette="Set1")
